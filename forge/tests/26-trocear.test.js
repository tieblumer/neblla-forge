...
export async function run({ reporter: r }) {
  ...
  // (Aubé migró del bloque de texto ```plan a la herramienta MCP `proponer_plan`:
  // el plan viaja como DATOS, determinista; el prompt pide partes + contrato +
  // complejidad. Ver tests/29-forge-plan.)
  {
    const pr = aubePrompt({ threadText: 'hola' });
    r.ok('el prompt manda usar la herramienta proponer_plan', /proponer_plan/.test(pr));
    r.ok('el prompt pide clasificar la complejidad', /complejidad/i.test(pr));
    r.ok('el prompt dice que por defecto una sola parte', /DEFECTO.*una sola parte/i.test(pr));
    r.ok('el prompt exige contrato entre partes', /contrato/i.test(pr) && /partes/i.test(pr));
  }
}
