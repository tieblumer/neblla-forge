...
import {
  subtareasDe, estadoPadre, diverge, decorar, ordenar, iconOf, ESTADOS, GRUPOS, pasoRecomendado,
...
export async function run({ reporter: r }) {
  ...
  // ── el PASO RECOMENDADO (la secuencia del ciclo, decidida EN LA FORJA) ───────
  // Importa que viva aquí: una IA orquestadora leerá `next` por API.
  {
    const P = (t) => (pasoRecomendado(t) || {}).key || null;
    r.eq('sin plan → rev', P({}), 'rev');
    r.eq('plan sin aprobar → aprobar', P({ plan: { aprobado: false } }), 'aprobar');
    r.eq('plan aprobado, sin tests → definir', P({ plan: { aprobado: true } }), 'definir');
    r.eq('tests definidos → escribir', P({ plan: { aprobado: true }, testsPlan: { tests: [{ id: 'T-1-01', estado: 'definido' }] } }), 'escribir');
    r.eq('tests escritos, sin build → ejecutar', P({ plan: { aprobado: true }, testsPlan: { tests: [{ id: 'T-1-01', estado: 'escrito' }] } }), 'ejecutar');
    r.eq('build sin correr → probar', P({ plan: { aprobado: true }, builtAt: 'x', testsPlan: { tests: [{ id: 'T-1-01', estado: 'escrito' }] } }), 'probar');
    r.eq('corrido, sin master → completar', P({ plan: { aprobado: true }, builtAt: 'x', testsPlan: { tests: [{ id: 'T-1-01', estado: 'pasa' }], ultimaCorrida: 'y' } }), 'completar');
    r.ok('en master → null (hecha)', pasoRecomendado({ plan: { aprobado: true }, builtAt: 'x', enMaster: true, testsPlan: { tests: [{ id: 'T-1-01', estado: 'pasa' }], ultimaCorrida: 'y' } }) === null);
    r.ok('decorar() incluye next', (decorar({ plan: { aprobado: true } }).next || {}).key === 'definir');
  }
